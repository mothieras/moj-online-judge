...
- **RabbitMQ** — 判题流程异步解耦（手动 ACK / 发布确认 / 重试 / 死信队列兜底）
- **Knife4j** — API 文档与在线调试
...
QuestionSubmitController        ← 用户提交代码
        ↓
QuestionSubmitServiceImpl       ← 写入 QuestionSubmit(WAITING)，投递 MQ
        ↓
RabbitMQ judge.queue            ← 异步解耦提交请求与判题执行
        ↓
JudgeConsumer                   ← 手动 ACK；按提交状态做幂等判断
        ↓
JudgeServiceImpl                ← 标记 RUNNING，编排判题
        ↓
...
| 场景 | 处理 |
|------|------|
| 发布到 Broker | 开启 publisher confirm，记录发送成功/失败 |
| 重复消费 | 消费前检查提交状态，仅 `WAITING` 进入判题，非 `WAITING` 直接 ACK |
| 用户代码错误 | 编译错误、运行错误、超时、答案错误直接写 `FAILED`，不重试 |
| 沙箱基础设施异常 | 重置为 `WAITING` 后按 `x-retry-count` 重试，最多 3 次 |
| 重试耗尽 | `basicNack(..., requeue=false)` 进入死信队列，由 `DeadLetterConsumer` 兜底标记 `FAILED` |
...
| **内部异常** (RuntimeException 等) | 意外 NPE、DB 错误 | 安全网：尽量将 RUNNING 提交置为 FAILED，消息进入死信队列 |

## 测试

当前单元测试覆盖 `QuestionSubmitServiceImpl`、`JudgeServiceImpl`、`JudgeConsumer`、`DeadLetterConsumer`、`RemoteCodeSandbox`、Redis 题目缓存等核心路径；最近一次 surefire 报告为 **39 tests, 0 failures, 0 errors**。
...
| Router / 工厂思想 | `judge/codesandbox/CodeSandboxRouter` | 根据 `codesandbox.type` 配置选择沙箱实现（`CodeSandboxFactory` 为旧静态工厂版本，已标记 `@Deprecated`） |
...
codesandbox:
  type: remote
  remote:
    url: http://localhost:8090/executeCode
